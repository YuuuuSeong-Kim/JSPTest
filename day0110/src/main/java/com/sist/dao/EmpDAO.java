...
public class EmpDAO {
	...
	public ArrayList<String> getMname(){
		...
		try {
			...
			System.out.println("getDname 예외발생 : "+e.getMessage());
		}
		return list;
	}
	
	public ArrayList<String> getDname(){
		String sql = "select dname from dept order by dno";
		ArrayList<String> list = new ArrayList<String>();
		try {
			Connection conn = ConnectionProvider.getConnection();
			Statement stmt = conn.createStatement();
	...
	public ArrayList<HashMap<String, Object>> findEmpAll(String category,String keyword, String op){
