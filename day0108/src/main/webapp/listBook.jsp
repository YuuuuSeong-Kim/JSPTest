	...
	$(function() {
		...
		$.ajax({
			...
			success : function(data) {
				$.each(data, function() {
					var select = $("#s_pub");
					select.append($("<option></option>").html(this).attr(
							"value", this));
					console.log($(this));
				})
			}
...
<body>
	...
	<form action="listBook.do">
		<select name="category" id="category">
			...
			<option value="<="><=</option>
			<option value="=">=</option>
		</select> : <select id="s_pub"></select><input type="search" name="keyword"
	...
	<table border="solid 1px">
		<tr>
			<c:if test="${category != null}">
				<th><a
					href="listBook.do?order=bookid&keyword=${keyword }&category=${category}&op=${op}&snum=${snum}">도서번호</a></th>
				<th><a
					href="listBook.do?order=bookname&keyword=${keyword }&category=${category}&op=${op}&snum=${snum}">도서명</a></th>
				<th><a
					href="listBook.do?order=price&keyword=${keyword }&category=${category}&op=${op}&snum=${snum}">가격</a></th>
				<th><a
					href="listBook.do?order=publisher&keyword=${keyword }&category=${category}&op=${op}&snum=${snum}">출판사</a></th>
			</c:if>
			<c:if test="${category == null}">
