...
public class ListBoard extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//이 서블릿은 html을 직접 응답하지 않고
		//게시물 목록을 조회하여 request 객체에 상태유지 한 다음
		//결과를 보여줄 jsp로 이동하도록 한다.
		BoardDAO dao = new BoardDAO();
		ArrayList<BoardVO> list = dao.findAll();
		request.setAttribute("list", list);
		
		//결과를 보여주기 위한 jsp로 이동시키기 위한 dispatcher 객체를 생성
		//생성시에 매개변수로 이동할 jsp이름을 써 준다.
		RequestDispatcher dispatcher = request.getRequestDispatcher("listBoard.jsp");
		
		//dispatcher 객체를 통하여 jsp로 이동한다.
